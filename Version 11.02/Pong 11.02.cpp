...
int midx,midy,m;//m=number of top scores available
...
void score_check(int n, int p)
{
	...
	for(j=0;j<i;j++)
	{
		outtextxy(menu_x2+menu_x1/2,menu_y1+30+(menu_y1/n)*(j),top[j]); 
	}
	clearmouseclick(WM_LBUTTONDOWN);
	while(!ismouseclick(WM_LBUTTONDOWN))
	{
...
int main( )
{
    initwindow(screen_end, screen_bottom, "Pong Are-Shor");
	begin:
...
void create_menu()
{
	...
	setcolor(text_color);
	settextstyle(FONT,HORIZ_DIR,FONT_SIZE);
	outtextxy(text,menu_y1+(menu_y1/2),"INSTRUCTIONS");
